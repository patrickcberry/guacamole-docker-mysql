...
otpw="$(cat guacamole-config/tmp-mysql-otpw.txt | xargs)"   # xargs to remove trailing whitespace
echo "otpw_file_size: $otpw_file_size"
echo "The OTWP:       $otpw"
...
# docker exec --privileged ras-mysql mysql --connect-expired-password -u root --password=\""$otpw"\" < ./resetrootpw.sql
docker exec --privileged ras-mysql mysql --connect-expired-password -u root --password=\""$otpw"\" -e \"ALTER USER \'root\'@\'localhost\' IDENTIFIED BY \'"$mysql_password"\'; FLUSH PRIVILEGES; EXIT;\"
